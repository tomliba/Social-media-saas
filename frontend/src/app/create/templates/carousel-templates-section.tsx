...
import { carouselTemplates, carouselThemes } from "@/lib/carousel-templates";
...
export default function CarouselTemplatesSection({ niche, tone }: { niche: string; tone: string }) {
  ...
  const fetchIdeas = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/generate-carousel-ideas", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          topic,
          niche,
          templateName: carouselTemplates.find((t) => t.id === selectedLayout)?.name || "Editorial",
        }),
      });
    ...
    <>
      ...
      {(step === "topic" || step === "ideas") && (
        <section ref={topicSectionRef} className="mb-16 max-w-xl animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <p className="text-on-surface-variant text-sm mb-4">We&apos;ll generate 10 carousel ideas based on your topic</p>
          ...
          <input
            ...
            placeholder="e.g., 5 habits that changed my life, productivity hacks for founders"
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className="text-[10px] font-bold text-on-surface-variant/50">{idea.slideCount} slides</span>
                        <span className={`px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase ${isChecked ? "bg-surface-container-low text-on-surface-variant" : "bg-surface-container-high/30 text-outline"}`}>
                          {idea.tag}
                        </span>
                      </div>
                    </div>

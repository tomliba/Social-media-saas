  ...
  const handleAnimateScenes = useCallback(async () => {
    const segments = editScenes
      .map((s, i) => {
        ...
        const dur = Math.min(10, Math.max(5, estimated));
        return { index: i, image_url: sceneImageUrls[i], motion_prompt: s.motion_prompt || "", duration: dur };
      })
      .filter((seg) => seg.image_url);

    if (hookImageUrl) segments.unshift({ index: -1, image_url: hookImageUrl, motion_prompt: editHookMotionPrompt || "slow atmospheric zoom in", duration: 5 });
    if (ctaImageUrl) segments.push({ index: -2, image_url: ctaImageUrl, motion_prompt: editCtaMotionPrompt || "gentle zoom out", duration: 5 });
